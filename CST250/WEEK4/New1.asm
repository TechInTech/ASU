...
NextSpot:
	...
	nop	
Branch0:
	#Do the addition here - $t3 is loaded
...
B0A:
	...
	jal get_char	# jump and link to label get_char (put char into $t1)
	nop		# This is the second operand
	bne $s4, $t1, B0A	# check if character from get_char was '='
	nop			# while(char != '=')
	addiu $t4, $t2, 0	# Transfers $t2 into $t4
...
StoreOperand1Plus:
	...
	jal get_char	# jump and link to label get_char (put char into $t1)
	nop		# This is the second operand
	bne $s5, $t1, StoreOperand1Plus	# check if character from get_char was '+'
	nop			# while(char != '=')
...
StoreOperand1Minus:
	...
	jal get_char	# jump and link to label get_char (put char into $t1)
	nop		# This is the second operand
	bne $s6, $t1, StoreOperand1Minus	# check if character from get_char was '-'
	nop				# while(char != '=')
	addiu $t3, $t2, 0	# Transfers $t2 into $t3
	move $t2, $0
	jr $ra		# return to caller
...
StoreOperand1Mult:
	...
	jal get_char	# jump and link to label get_char (put char into $t1)
	nop		# This is the second operand
	bne $s7, $t1, StoreOperand1Mult	# check if character from get_char was '*'
	nop				# while(char != '=')
	addiu $t3, $t2, 0	# Transfers $t2 into $t3
	move $t2, $0
	jr $ra		# return to caller
